...
///
/// Parsed in `TerminalRepresentable` via `Terminal.registerOscHandler`.
enum ShellIntegration {
    ...
    private static func zshPlan(in dir: URL) -> Plan {
        ...
        return Plan(args: [], env: ["ZDOTDIR": zdotdir.path, "SWIFTTERMINAL_SHELL_INTEGRATION": "1"])
    }
    ...
    private static func bashPlan(in dir: URL) -> Plan {
        ...
        return Plan(args: ["--rcfile", rc.path, "-i"], env: ["SWIFTTERMINAL_SHELL_INTEGRATION": "1"])
    }
    ...
    private static func supportDirectory() -> URL? {
        ...
        let bundleName = Bundle.main.bundleIdentifier ?? "SwiftTerminal"
        let dir = support
    ...
    private static func passthrough(name: String) -> String {
        """
        # SwiftTerminal shell integration: forward to the user's \(name)
        if [ -f "$HOME/\(name)" ]; then
            . "$HOME/\(name)"
        fi
        """
    ...
    # SwiftTerminal shell integration (OSC 133). Forwards to the user's .zshrc, then
    ...
    __swiftterminal_precmd() {
        local exit=$?
        printf '\e]133;D;%d\a' "$exit"
    }
    __swiftterminal_chpwd() {
        printf '\e]7;file://%s%s\a' "${HOST:-localhost}" "$PWD"
    }

    if (( $+functions[add-zsh-hook] )); then
        add-zsh-hook preexec __swiftterminal_preexec
        add-zsh-hook precmd __swiftterminal_precmd
        add-zsh-hook chpwd __swiftterminal_chpwd
    fi
    # Emit once at startup so the host learns the initial directory.
    __swiftterminal_chpwd
    """#
    ...
    # SwiftTerminal shell integration (OSC 133). Source the user's bash startup files
    ...
    __swiftterminal_preexec_invoke() {
        ...
        __swiftterminal_preexec_ran=1
        printf '\e]133;C;%s\a' "$BASH_COMMAND"
        return $ret
    }
    __swiftterminal_precmd_invoke() {
        local ret=$?
        printf '\e]133;D;%d\a' "$ret"
        printf '\e]7;file://%s%s\a' "${HOSTNAME:-localhost}" "$PWD"
        __swiftterminal_preexec_ran=0
        return $ret
    }

    trap '__swiftterminal_preexec_invoke' DEBUG
    case "$PROMPT_COMMAND" in
      *__swiftterminal_precmd_invoke*) ;;
      *) PROMPT_COMMAND="__swiftterminal_precmd_invoke${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
    esac
    # Emit once at startup so the host learns the initial directory.
    printf '\e]7;file://%s%s\a' "${HOSTNAME:-localhost}" "$PWD"
    """#
    ...
    function __swiftterminal_postexec --on-event fish_postexec
        printf '\e]133;D;%d\a' $status
    end
    function __swiftterminal_cwd --on-variable PWD
